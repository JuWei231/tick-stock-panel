...
出厂自带 4 个内置扩展表, 启动时**只创建 `config.json`, 不拉取数据**(`pull.enabled=False`, 见 #199), 数据由页面上的「获取数据」按钮或「数据」页的拉取面板手动触发:

| 表 id | 名称 | 上游接口 (`shy313.com/api/plugins/market_flow/exports/`) | 模式 | 入口 |
| :--- | :--- | :--- | :--- | :--- |
| `ext_gn_ths` | 扩展概念 | `ths-concepts` | snapshot | 概念分析页 |
| `ext_hy_ths` | 扩展行业 | `ths-industries` | snapshot | 行业分析页 |
| `ext_rq_ths` | 人气排行 | `popularity` | snapshot | 内置菜单「人气排行」 |
| `ext_zjlx_ths` | 资金流向 | `money-flow` | snapshot | 内置菜单「资金流向」 |

后两张表没有对应的内置页面, 由 `analysis.py` 的 `_default_menus()` 直接声明为**内置榜单菜单**(template `ranking`, 按排名升序), 不写用户数据、不可被误删、随代码升级;需要自定义列或配色时, 在「设置 → 扩展页面」用同 id 新建一个菜单即可覆盖。

单位口径 (接口边界已归一, 勿二次换算):

- 两张表的 `涨跌幅` 是**百分数值** (`4.03` 表示 `+4.03%`), 实测与本地 enriched 收盘价涨跌幅 corr=1.000000、中位差 0.000000 个百分点;字段名刻意避开 `change_pct`, 以免被当成实时入口的「小数制」口径复用;
- `资金流向` 的 `净流入`/`流入`/`流出` 单位为**元**, 且 `净流入 = 流入 - 流出`;
- 上游对停牌等标的会返回 `null` (资金三项各约 364 行), 展平时**保留空值不补零** —— 补零会把「无数据」伪装成「0 净流入」;
- 上游是个人维护的低频接口, 实测会连续返回 502, 因此 `_fetch_json` 对 5xx 与网络错误做 3 次退避重试(4xx 不重试)。
