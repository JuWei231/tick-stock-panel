...
每个数据集(日K/除权因子/实时/分钟/五档/财务/全量分钟)可**独立路由**到不同来源;未声明该数据集的源会自动回退 TickFlow。出厂自带的第三方源:

| 源 | 覆盖 | 前置条件 |
| :--- | :--- | :--- |
| fuyao(同花顺官方 REST) | 实时、日K、除权因子、财务 | 配置 `FUYAO_API_KEY` |
| stock-sdk | 日K、除权、分钟、实时 | Node 18+ 与 `npm install`;Docker 默认不打包;抓取第三方站点,合规风险自负 |
| **通达信** | 日K、除权因子、分钟、全量分钟、实时、**五档**、财务(仅股本表) | 无(直连行情主站,不需 Key) |

> **通达信**为直连第三方行情主站的社区反向工程协议, 使用前请自行评估合规责任。主站可用率低且会随时失效,插件内置了主站池与自动切换;首次使用建议在设置页点「试拉」验证连通性。它是目前唯一能提供 **五档盘口(`depth5`)** 的可选源。其财务只提供**股本表**(逐日总股本/流通股本, 由股本变迁事件还原):本机装了通达信客户端时优先直接读它的 `T0002\hq_cache\gbbq`(离线、秒级、含北交所), 未覆盖的标的才走主站;非默认安装路径可用环境变量 `TDX_HOME` 指定。
