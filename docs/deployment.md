...
**整个 `data/` 目录都不纳入 git** —— 行情 K线、财务、自选、回测、监控记录,乃至概念/行业/人气排行/资金流向扩展数据,全部是程序运行时生成/拉取的用户数据,`git pull` 物理上无法影响它们。新用户首次启动时只会**创建**这几份内置扩展表的配置(不拉取数据,见 #199),需要在对应页面点「获取数据」或到「数据」页手动拉取一次。
