...
- **`backend/app/plugins/tdx/`** — 通达信行情主站直连(Python 零依赖, 不需 Key)
  - 提供 `daily`(不复权原始价, 可回溯至 2003 年)、`adj_factor`(由除权除息事件按交易所公式推导)、`minute`/`full_minute`(1/5 分钟, 1 分钟约 27 个交易日)、`realtime`(全市场约 3.3s/轮)、`depth5`、`financial`(**仅 `shares`**, 由股本变迁事件还原逐日股本; 三大报表通达信没有, 其余财务表返回空); **不声明** `instruments`(缺名称/上市日/股本维表字段)
  - `protocol.py` — 协议层: 报文构造与响应解析(变长整数、压缩浮点、K线增量价格、指数多 4 字节涨跌家数)
  - `client.py` — 连接管理 + 主站池 + 批量坏记录隔离
  - `provider.py` — Provider 实现与单位归一
  - `gbbq.py` + `gbbq_keys.py` — 本地 `T0002\hq_cache\gbbq` 读取(离线/首刷股本来源)。`get_financials("shares")` 是**本地优先 + 在线补齐**: 本地覆盖的标的直接出阶梯(不发请求), 未覆盖的才逐只走主站; 文件缺失/损坏/过期(**最新事件日**早于今天 5 天)时整体回退在线, 与只有在线时行为一致。定位顺序: `TDX_HOME`/`TDX_DATA_DIR`(可指安装目录、`T0002` 或 `gbbq` 文件本身)→ 各盘根目录下常见安装目录名(多装取最新)。**本地市场号 2 = 北交所**(在线协议把 `.BJ` 映射到市场 0, 主站对市场 0 的北交所代码返回空事件), 本地路径因此补上在线拿不到的 343 只北交所股本。`gbbq_keys.py` 是机器生成的 4176 字节密钥表, 与客户端版本相关, 换表后需重新提取(解不出记录会 fail-closed 回退在线)
  - `tests/test_tdx_provider.py` — 68 个契约测试, 协议层用真实抓取的响应字节做固定样本(无需网络)
  - `tests/test_tdx_gbbq.py` — 本地 gbbq 测试: 合成文件加解密往返(测试侧实现加密的逆运算)、类别槽位口径与在线同序、市场号映射、离线路径零联网、在线补齐、过期/损坏回退、快照缓存失效、真实文件结构与平台股本表 parity(本机无通达信安装时真实文件用例跳过)
  - **实现要点(踩过的坑)**: 主站可用率极低(实测约 18 个可连主站仅 1 个提供行情)且会失效, 因此主站池与自动切换是必需项而非优化; 快照响应条数在 `offset 2` 而K线在 `offset 0`; 指数必须按"多 4 字节涨跌家数"解析; 基金/ETF 的快照价格除数是 1000 而个股是 100(按 100 解析会整体放大 10 倍); 退市标的返回短记录, 必须做边界保护并二分拆包, 否则一条坏记录会丢掉整批 80 只
