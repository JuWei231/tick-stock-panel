...
GOLDEN_COLUMNS: list[dict] = [
    ...
    {"id": "log_float_mv", "label": "流通市值对数", "group": "规模", "desc": "ln(不复权收盘价 x 当日成交量 / 换手率), 由换手率反推流通股本, 高值代表大盘"},
...
GOLDEN_VIRTUAL_DEPS: dict[str, frozenset[str]] = {
    ...
    **{
    ...
    # --- 扩充批次 (2026-09-05) ---
    # 市值口径必须用不复权价: close 是前复权价, 乘当日股本会把复权比计入两次
    "log_float_mv": frozenset({"raw_close", "volume", "turnover_rate"}),
